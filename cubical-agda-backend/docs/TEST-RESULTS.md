...
| `make verify-three-lane-e2e` | one real binding-time analysis; locked Stock Agda/MAlonzo/GHC native, separate v2 producer/consumer packet, and Stock-MAlonzo linked runtime NbE; mismatch/resource/cancel/stale cleanup | 3 real positives PASS; type mismatch, module identity mismatch, fuel exhaustion and TERM cancellation EXPECTED-REJECT; no stale cross-lane publication | Linux clean clone and macOS run `32829677728` PASS |
