...
Goal 3 has code and specialized tests for 11/11 implementation items, but the current complete clean-clone run is pending and the goal is not accepted. The runtime passes 27 self-tests;
...
| `make verify` | aggregate native/provider/bridge/MAlonzo/differential plus legacy gates | PENDING: the current workspace lacks the pruned legacy Agda dynamic dependency closure; the new macOS workflow performs an actual second clone, provisions both matching toolchains and runs this aggregate target. No completion claim until that run is green. | current change |
