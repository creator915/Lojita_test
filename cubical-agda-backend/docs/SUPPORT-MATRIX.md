...
| Goal 3 NbE linked into the final program process | `OWNER-BLOCKED` | 11/11 technical evidence: pinned cctt Core eval/quotation normalizes actual Bool/Int/Vec/Sigma inputs and the proof-linked exact same-input gate passes 6/6 in locked PR/push CI. Independent acceptance is pending; general Cubical normalization remains outside the claim. |
...
| Final-process runtime NbE | `VERIFIED` | For the declared fragment, checked Internal Bool/Nat/Int/Vec/Pi/Sigma/Glue/PathP/S¹ definitions translate without compiler normalization and execute through a cctt-gated runtime in a Stock Agda/MAlonzo/GHC final program under a no-exec guard. This row does not imply general Cubical semantic coverage. |
